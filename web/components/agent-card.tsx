...
export function AgentCard({
  ...
  return (
    <button
      ...
      className={cn(
        "group relative flex h-full flex-col rounded-[16px] border border-graphite-rail bg-void-black p-6 text-left transition-colors duration-150 ease-out",
        "hover:border-smoke focus:outline-none focus-visible:border-electric-blue/70",
      )}
    >
      {/* Top row: identity + verification mark. */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5">
          <StatusDot status={entry.status} className="mt-px" />
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-[16px] font-semibold leading-tight text-pure-white">
                {card.name}
              </span>
              {entry.visibility === "private" && (
                <Lock className="size-3 text-steel" aria-label="private" />
              )}
            </div>
