    ...
    vec![
        tool(
            ...
            json!({
                ...
                "preapprove": { "type": "array", "items": { "type": "string" }, "description": "trusted names/ids to auto-admit with no prompt (e.g. [\"codex\"]); everyone else still needs approval. A leaked key can't admit anyone off-list." },
...
mod tests {
    ...
    /// ~12,970; ceiling → 13,200 to keep ~20% headroom against real regressions. Adding
    /// `parler_open_session`'s `preapprove` param (auto-admit trusted joiners, #108) grew it ~230 B to
    /// ~13,200; ceiling → 13,400 to restore a small regression margin.
    const TOOL_SPECS_BUDGET: usize = 13_400;
